...
import { RefreshCw, Package, Users, Plus, X, Lock, LogOut, Edit3, ChevronDown, ChevronRight, Edit2, GripVertical, ArrowUp, ArrowDown } from 'lucide-react';
...
export default function Admin() {
  ...
  const moveCat = async (id, direction) => {
    const idx = orderedCats.findIndex(c => c.id === id);
    if (idx < 0) return;
    ...
    const withOrder = newCats.map((cat, i) => ({ ...cat, displayOrder: i + 1 }));
                    ...
                    >
                        <div className="p-5 flex justify-between items-center hover:bg-gray-50 transition-colors border-b border-gray-100">
                         <div className="flex items-center space-x-3">
                             ...
                             >
                               <div
                                 className="cursor-grab active:cursor-grabbing text-gold-500 hover:text-gold-600 transition-all p-2 bg-gold-50 rounded-lg flex items-center space-x-1 group"
                                 onClick={e => e.stopPropagation()}
                                 title="Click and drag to reorder categories"
                               >
                                 <GripVertical className="w-5 h-5 group-active:scale-110" />
                                 <span className="text-[10px] font-black uppercase tracking-tighter hidden sm:block">Reorder</span>
                               </div>
                               <button 
                                 onClick={(e) => { e.stopPropagation(); moveCat(cat.id, 'up'); }}
                                 disabled={catIdx === 0}
                                 className={`p-1 rounded-md transition-colors ${catIdx === 0 ? 'text-gray-200 cursor-not-allowed' : 'text-forest-400 hover:bg-forest-50 hover:text-gold-500'}`}
                                 title="Move Up"
                               >
                               ...
                               <button 
                                 ...
                                 disabled={catIdx === orderedCats.length - 1}
                                 className={`p-1 rounded-md transition-colors ${catIdx === orderedCats.length - 1 ? 'text-gray-200 cursor-not-allowed' : 'text-forest-400 hover:bg-forest-50 hover:text-gold-500'}`}
                                 title="Move Down"
                               >
